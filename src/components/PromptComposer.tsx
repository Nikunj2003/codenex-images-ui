...
export const PromptComposer: React.FC = () => {
  ...
  const handleGenerate = () => {
    ...
    if (selectedTool === 'generate') {
      ...
      console.log('[PromptComposer] Generating with:', {
        prompt: currentPrompt.substring(0, 100),
        uploadedImagesCount: uploadedImages.length,
        referenceImagesCount: referenceImages.length,
        referenceImageSizes: referenceImages.map(img => img.length),
        temperature,
        seed
      });
  ...
  return (
    <>
    <div className="w-80 lg:w-72 xl:w-80 h-full bg-gray-900/50 backdrop-blur-xl border-r border-gray-800/30 p-6 flex flex-col space-y-6 overflow-y-auto">
      {/* Gradient overlay */}
      <div className="absolute inset-0 bg-gradient-to-b from-accent-600/5 via-transparent to-codenex-600/5 pointer-events-none" />
      <div className="relative">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-bold text-gray-200 uppercase tracking-wider">Mode</h3>
        ...
        <div className="grid grid-cols-3 gap-2">
          {tools.map((tool) => (
            <button
              ...
              className={cn(
                'flex flex-col items-center p-3 rounded-xl border transition-all duration-300 group',
                selectedTool === tool.id
                  ? 'bg-gradient-to-br from-accent-500/20 to-codenex-500/20 border-accent-400/50 text-accent-400 shadow-lg shadow-accent-500/20'
                  : 'bg-gray-800/50 border-gray-700/50 text-gray-400 hover:bg-gray-800 hover:text-gray-300 hover:border-gray-600'
              )}
            >
              <tool.icon className="h-5 w-5 mb-1 group-hover:scale-110 transition-transform" />
      ...
      {/* Modern Prompt Input Card */}
      <div className="bg-gray-800/30 backdrop-blur-sm rounded-xl p-4 border border-gray-700/30">
        <label className="text-sm font-bold text-gray-200 mb-3 block uppercase tracking-wider">
          {selectedTool === 'generate' ? 'Create' : 'Edit'}
        </label>
        <Textarea
          value={currentPrompt}
          ...
          className="min-h-[120px] resize-none bg-gray-900/50 border-gray-700/50 focus:border-accent-500/50"
        />
        ...
        >
          {currentPrompt.length < 20 ? (
            ...
            <div className={cn(
              ...
              currentPrompt.length < 50 ? 'bg-amber-500' : 'bg-codenex-500'
            )} />
          )}
      ...
      {/* Simple Generate Button with Clean Highlight */}
      <div className="relative">
        <Button
          onClick={handleGenerate}
          disabled={isGenerating || !currentPrompt.trim()}
          className={`relative w-full h-14 text-base font-semibold rounded-lg transition-all duration-200 ease-out
            ${isGenerating || !currentPrompt.trim() 
              ? 'bg-gray-800 text-gray-500 cursor-not-allowed' 
              : 'bg-gray-900 text-white hover:bg-purple-900/20 hover:border-purple-500/50 border border-gray-700 hover:text-purple-200 cursor-pointer group'
            }
          `}
        >
          {/* Simple highlight overlay */}
          <div className="absolute inset-0 bg-gradient-to-r from-purple-600/0 via-purple-600/10 to-purple-600/0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-lg"></div>
          ...
          <div className="relative flex items-center justify-center">
            {isGenerating ? (
              <>
                ...
                <span>{selectedTool === 'generate' ? 'Generating...' : 'Editing...'}</span>
              </>
            ) : (
              <>
                <Wand2 className={`h-5 w-5 mr-2 transition-colors duration-200 ${
                  currentPrompt.trim() ? 'text-purple-400 group-hover:text-purple-300' : 'text-gray-600'
                }`} />
